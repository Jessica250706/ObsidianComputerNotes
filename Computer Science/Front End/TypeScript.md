...
[【三小时快速上手TypeScript，TS速通教程】](https://www.bilibili.com/video/BV1YS411w7Bf?vd_source=4e42d3c23020c1c6dc6a9aac2d11ab9c)

# 1.TypeScript 快速上手（尚硅谷）
...
- 现如今的 JavaScript **应用场景**⽐当年**丰富**的多，**代码量**也⽐当年大很多，随便一个 JavaScript 项目的代码量，可以轻松的达到几万行，甚至十几万行！
...
> 1. ⼯程中会生成一个 `tsconfig.json` 配置文件，其中包含着很多编译时的配置。
...
1. **原始类型 VS 包装对象**
	...
	- 包装对象：如 `Number` 对象、 `String` 对象、 `Boolean` 对象，是复杂类型，在内存中占用更多空间，在日常开发时很少由开发人员自⼰创建包装对象。
...
3. 那限制函数返回值时，是不是 `undefined` 和 `void` 就没区别呢?—— 有区别。因为还有这句话 ：【返回值类型为 **`void`** 的函数，调用者不应依赖其返回值进行任何操作！】对⽐下面两段代码：
...
关于 **`object`** 与 **`Object`** ，直接说结论：实际开发中用的==相对较少==，因为范围太大了。

#### 5.1 object(小写)

**`object`** (小写)的含义是：所有**非原始类型**，可存储：对象、函数、数组等，由于限制的范围**比较宽泛**，在实际开发中使用的**相对较少**。

p.s. 实际开发中，`object` 可以用在 `vue3` 父子组件通信，在子组件（往往是你抽象封装的组件），允许父组件传入任何 `object` 类型数据，子组件读取，渲染上去。
...
2. 索引签名： 允许定义对象可以具有==任意数量的属性==，这些属性的==键==和==类型==是可变的，常用于：描述类型不确定的属性，(具有动态属性的对象)。
...
  name: string
  age?: number
  [key: string]: any // 索引签名，完全可以不用key这个单词，换成其他的也可以(另一种常用写法：index)
...
**备注：**
- `TypeScript` 中的 => 在函数类型声明时表示==函数类型==，描述其==参数类型==和==返回类型==。（不是箭头函数！）
...
> 元组 (Tuple) 是一种==特殊的数组类型==，可以存储==固定数量==的元素，并且每个元素的类型是==已知==的且可以==不同==。元组用于精确描述一组值的类型， `?` 表示可选元素。

```ts
// 第一个元素必须是 string 类型，第⼆个元素必须是 number 类型。
let arr1: [string, number]

// 第一个元素必须是 number 类型，第⼆个元素是可选的，如果存在，必须是 boolean 类型。
...
比如 React 的 `useState` Hook 返回的就是一个元组：`[状态值, 修改状态的函数]`。
...
如果你自己写一个解析函数，返回解析后的值和可能的错误：

```ts
function parseInput(input: string): [number, Error | null] {
    const num = parseInt(input, 10);
    if (isNaN(num)) {
        return [0, new Error('Invalid number')];
    }
    return [num, null];
}

const [result, error] = parseInput('123');
if (error) {
    console.error(error.message);
} else {
    console.log(result); // result 在这里类型被推断为 number
}
```

##### 场景二：处理 CSV / Excel 等行列数据

处理 CSV 文件时，每一行都是一个固定结构的元组。

```ts
// 定义一行CSV数据的结构: [姓名, 年龄, 是否激活]
type UserRow = [string, number, boolean];

// 从文件读取的原始数据
const csvData: UserRow[] = [
    ['Alice', 30, true],
    ['Bob', 25, false],
    ['Charlie', 35, true],
];
...
// 类型安全的遍历
csvData.forEach(([name, age, isActive]) => {
    // 这里 name 是 string，age 是 number，isActive 是 boolean
    console.log(`${name} is ${age} years old and is ${isActive ? 'active' : 'inactive'}.`);
});
```

##### 场景三：轻量级的数据结构（替代对象）

当数据很简单，不需要为它专门定义一个接口或类时，元组是非常轻量、便捷的选择。比如一个坐标点或一个键值对。
...
##### 场景四：剩余参数（Rest Parameters）

当你写一个函数，可以接收任意数量的参数，但这些参数的类型有特定规律时，可以用元组来定义剩余参数的类型。

```ts
// 定义一个函数，第一个参数是字符串，后面可以跟任意多个数字
function logWithNumbers(prefix: string, ...numbers: [number, ...number[]]) {
    console.log(prefix, numbers.join(', '));
}

logWithNumbers('Numbers:', 1, 2, 3); // 正确
logWithNumbers('Numbers:', 1, 'a');  // 错误：'a' 不是 number 类型
```

#### 高级用法：带标签的元组

为了提升代码可读性，TypeScript 4.0+ 支持给元组的元素加上“标签”。

```ts
// 不加标签
function useData(): [string, number, boolean] { 
    // ...
}

// 加标签（只影响编辑器和文档提示，不影响实际类型）
function useData(): [name: string, age: number, isActive: boolean] {
    // ...
}

const [name, age, active] = useData();
// 解构时，编辑器会提示 name, age, active 对应的标签，非常清晰
```
...
如下代码的功能是：根据调用 `walk` 时传入的不同参数，执行不同的逻辑，存在的问题是调用 `walk` 时传参时没有任何提示，编码者很容易写错字符串内容；并且用于判断逻辑的 `up` 、 `down` 、 `left` 、 `right` 是连续且==相关的一组值==，那此时就特别适合使用==枚举( `enum` )==。
...
数字枚举是一种最常见的枚举类型，其成员的值会==自动递增==，且数字枚举还具备==反向映射==的特点，在下面代码的打印中，不难发现：可以通过==值==来获取对应的枚举==成员名称==。
...
`type` 可以为==任意类型==创建==别名==，让代码更简洁、可读性更强，同时能更方便地进行类型复用和扩展。
...
- 简记：抽象类==不能实例化==，其意义是==可以被继承==，抽象类⾥可以有==普通方法==、也可以有==抽象方法==。
...
  abstract calculate(): number // 通用方法:打印包裹详情
  printPackage() {
...
let p 1: PersonInterface
let p 2: PersonInterface

p 1 = { name: '张三', age: 18, extraInfo: '一个好人' }
...
const p2 = new Person("tom", 30, { title: '研发总监', company: '发发发科技公司' });
```
...
> 备注：虽然 `TypeScript 5.0` 中可以直接使用 **`类装饰器`**，但为了确保其他装饰器可用，现阶段使用时，仍建议使用 `experimentalDecorators` 配置来开启装饰器支持，而且不排除在来的版本中，官方会**进一步调整**装饰器的相关语法！
> 
...
// console.log(p 1) // 打印的 p1 是：_classThis，转换的 JS 版本比较旧时，会出现，不必纠结
p1.speak();
p1.introduce();
```
...
装饰器可以组合使用，执行顺序为：先【由上到下】的执行所有的装饰器工厂，依次获取到装饰器，然后再【由下到上】执行所有的装饰器。（洋葱模型）
...
function State(target: object, propertyKey: string) {
  ...
  Object.defineProperty(target, propertyKey, {
    ...
    set(newVal: string) {
      ...
      this[key] = newVal; // 把这个值放在实例的自身。当实例调用原型上的方法、属性时，其 this 指向的就是实例自身
    },
    enumerable: true,
    configurable: true,
  });
